...
namespace NinjutsuGames.StateMachine.Runtime
{
    ...
    public abstract class BaseNode
    {
        ...
        [NonSerialized] public GameObject context;

        protected List<int> IsContextRunning { get; } = new();
        protected List<int> IsContextDisabled { get; } = new();
        
        protected int NodeId(GameObject context) => context.GetInstanceID() + GUID.GetHashCode();
        ...
        public bool CanExecute(GameObject context) => enabledForExecution && 
                                                      (!context || (!IsContextDisabled.Contains(NodeId(context)) && context.activeInHierarchy && context.Get<StateMachineRunner>().enabled));
